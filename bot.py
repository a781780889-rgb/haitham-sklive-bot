...
async def generate_and_send_pdf(update, context, uid):
    ...
    try:
        ...
        pre_gsl_code = db.generate_medical_report_code(hospital_type=hospital_type_val or "حكومي")
    ...
    try:
        ...
        generate_companion_pdf(
            companion_data = {
                ...
                "companion_name_en": pc_data.get("companion_name_en", ""),
        ...
        await message.reply_document(
            ...
            caption=(
                ...
                f"📋 *بيانات المريض:*\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"- الاسم: {pc_data.get('companion_name', '—')}\n"
                f"- رقم الهوية: `{pc_data.get('id_number', '—')}`\n"
                f"- الجنسية: {pc_data.get('nationality', '—')}\n"
                f"- جهة العمل: {pc_data.get('workplace', '—')}\n"
                f"- تاريخ بدء الإجازة: {pc_data.get('admission_date', '—')}\n"
                f"- عدد الأيام: {pc_data.get('days_count') or '—'}\n"
                f"- تاريخ الإصدار: {datetime.now().strftime('%d-%m-%Y')}\n"
                f"- وقت الإصدار: {datetime.now().strftime('%H:%M')}\n\n"
                f"🔍 *للتحقق:*\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🔑 رمز الإحالة: `{gsl_code}`\n\n"
                f"🌐 رابط التحقق:\n"
                f"[www.seha.sa/#/inquiries/slenquiry]({website_url})\n\n"
                f"💡 *ملاحظة:* انتظر 3 دقائق قبل التحقق من ظهور البيانات."
            ),
            parse_mode="Markdown",
        )
