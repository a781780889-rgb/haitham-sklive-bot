...
واجهة الاستعلام: HTML/CSS مبنية على تصميم منصة صحة الرسمي
"""
...
def get_bg_b64():
    p = os.path.join(_THIS_DIR, "232971.jpg")
    if os.path.exists(p):
        with open(p, "rb") as f:
...
html,body { font-family:'Tajawal',sans-serif; direction:rtl; min-height:100vh; }
...
.spinner {
  width:52px; height:52px; border:4px solid #bbdefb;
  border-top:4px solid #1565c0; border-radius:50%; animation:spin .8s linear infinite;
}
@keyframes spin { to{ transform:rotate(360deg); } }
.loading-text { font-size:16px; color:#1565c0; font-weight:700; }
</style>
</head>
<body>
...
async function doQuery() {
  ...
  try {
    await fetch('/api/verify?gsl='+encodeURIComponent(gsl)+'&id='+encodeURIComponent(id));
  } catch(e) {}
...
</html>""".replace("BG_PLACEHOLDER", bg)
